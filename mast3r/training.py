...
from mast3r.inference import loss_of_one_batch  # noqa
...
def get_args_parser():
    ...
    parser.add_argument('--train_criterion_dg', default='FocalLoss()', type=str, help="train criterion for doppelganger prediction")
    parser.add_argument('--test_criterion', default=None, type=str, help="test criterion")
    parser.add_argument('--test_criterion_dg', default=None, type=str, help="test criterion for doppelganger prediction")
...
def test(args):
    ...
    data_loader_test = {dataset.split('(')[0]: build_dataset(dataset, 1, args.num_workers, test=True)
                        for dataset in args.test_dataset.split('+')}
    
    print('Loading model: {:s}'.format(args.model))
    model = eval(args.model)
    print(f'>> Creating test criterion = {args.test_criterion_dg}')
    test_criterion_dg = eval(args.test_criterion_dg).to(device)
    ...
    for test_name, testset in data_loader_test.items():
        print(f"Evaluating on dataset: {test_name}")
        ...
        if match:
            teston_content = [item.strip("'\"") for item in match.group(1).split(",")]
            print(f"Content of teston: {teston_content}")
        else:
...
def train(args):
    ...
    print('Loading model: {:s}'.format(args.model))
    model = eval(args.model)
    print(f'>> Creating train criterion = {args.train_criterion} and {args.train_criterion_dg}')
    train_criterion = eval(args.train_criterion).to(device)
    train_criterion_dg = eval(args.train_criterion_dg).to(device)
    print(f'>> Creating test criterion = {args.test_criterion or args.train_criterion} and {args.test_criterion_dg or args.train_criterion_dg}')
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if (epoch > 0 and args.eval_freq > 0 and epoch % args.eval_freq == 0):
            test_stats = {}
            for test_name, testset in data_loader_test.items():
                stats = test_one_epoch(model, test_criterion, test_criterion_dg, testset,
        ...
        train_stats = train_one_epoch(
            model, train_criterion, train_criterion_dg, data_loader_train,
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, criterion_dg: torch.nn.Module,
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        loss_tuple = loss_of_one_batch(batch, model, 
...
@torch.no_grad()
def test_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, criterion_dg: torch.nn.Module,
